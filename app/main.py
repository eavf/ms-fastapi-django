...
def verify_auth(authorization = Header(None), settings: S = Depends(get_settings)):
    """
...
async def predictions_view(file:UploadFile = File(...), authorization = Header(None), settings:Settings = Depends(get_settings)):
